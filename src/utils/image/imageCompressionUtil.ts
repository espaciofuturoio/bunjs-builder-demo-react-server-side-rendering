...
import { isHeic as checkIsHeic, heicTo } from 'heic-to'
// Import CSP version for Content Security Policy environments
import { isHeic as checkIsHeicCsp, heicTo as heicToCsp } from 'heic-to/csp'
...
export const optimizeImageServer = async (
	...
	try {
		...
		const response = await fetch(`${API_BASE_URL}/api/upload/optimize`, {
			method: 'POST',
			body: formData,
		})
...
/**
 * Compresses and optimizes an image buffer using Sharp
 */
export async function optimizeImage(
	...
	// Process the image
	const data = await sharpInstance.toBuffer({ resolveWithObject: true })
